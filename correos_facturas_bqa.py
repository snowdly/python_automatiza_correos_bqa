...
if Continuar_Ventana:
    ...
    for elemento in PDFS:
        ...
        if rr['Resultado'] == 'OK':
            ...
            try:
                procesos_comunes.pdf_renombra_mueve(elemento, vaceptacion['Acta_Aceptacion'] + '-' + vaceptacion['N_Pedido']
                                                    , carpetas_trabajo['Aceptacion'])
            except Exception as e1:
                print('No se ha podido mover el fichero {}, error {}'.format(elemento, e1))
        else:
            print(rr['Error'])
    ...
    print(lconsolidado)
    # 4. Generar el excel consolidado
    df = pd.DataFrame(lconsolidado, columns=['N_Pedido', 'Acta_Aceptacion', 'Importe', 'Descripcion',
                                             'Cantidad', 'Precio_Unitario', 'Subtotal',
                                             'Fecha_Aceptacion', 'Fecha_Grabacion'])
    df['Importe'] = df['Importe'].astype('float32')
    df['Cantidad'] = df['Cantidad'].astype('float32')
    df['Precio_Unitario'] = df['Precio_Unitario'].astype('float32')
    df['Subtotal'] = df['Subtotal'].astype('float32')
    writer = pd.ExcelWriter(os.path.join(carpetas_trabajo['Aceptacion'],
                                         'Aceptaciones_' + procesos_comunes.convertir_fecha_nombre() + '.xlsx'),
                            engine='xlsxwriter')
    df.to_excel(writer, sheet_name='Pedidos', index=False)
    writer.save()
